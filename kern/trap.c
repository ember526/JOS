...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 0x03) == 0) {
		print_trapframe(tf);
		panic("Page fault in kernel mode ：eip:0x%x va : 0x%x", tf->tf_eip, fault_va);
	}
	...
	if (curenv->env_pgfault_upcall) {
		uint32_t *stacktop = (uint32_t *)UXSTACKTOP - 1;
		if(tf->tf_esp>=UXSTACKTOP-PGSIZE && tf->tf_esp<UXSTACKTOP) {//fault env already in exception stack
			stacktop = (uint32_t *)tf->tf_esp - 1;
			cprintf("stacktop 0x%x\n", stacktop);
			*stacktop = 0;
			stacktop = (void *)stacktop - sizeof(struct UTrapframe);
			//cprintf("before 0x%x\n",stacktop);
			//cprintf("after 0x%x\n",stacktop);
		}
		else 
		...
		//store necessary data since they will be overwritten
		user_mem_assert(curenv, stacktop, sizeof(struct UTrapframe),PTE_U|PTE_P|PTE_W );
